...
module('Unit: ExponentialBackoffPolicy', function () {
  ...
  test('uses exponentially constructed delays', function (assert) {
    ...
    assert.deepEqual(
      ...
      'expected to have exponentially expanded delay timings'
    );
  });
  ...
  test('#shouldRetry checks the retry count against number of delay steps', function (assert) {
    ...
    assert.notOk(
      backoffPolicy.shouldRetry({ retryCount: 6 }),
      'expected to be told not to retry'
    );
    assert.notOk(
      backoffPolicy.shouldRetry({ retryCount: 7 }),
      'expected to be told not to retry'
    );
    assert.ok(
      backoffPolicy.shouldRetry({ retryCount: 1 }),
  ...
  test('#shouldRetry should check if the reason matches what we want to retry', function (assert) {
    ...
    assert.ok(
      backoffPolicy.shouldRetry({ retryCount: 1 }, new FakeError('hello')),
      'expected to be told to retry'
    );
    assert.ok(
      backoffPolicy.shouldRetry({ retryCount: 1 }, 'foo message'),
      'expected to be told to retry'
    );
    assert.notOk(
      backoffPolicy.shouldRetry(
        { retryCount: 1 },
